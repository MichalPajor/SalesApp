...
namespace SalesApp.ViewModels
{
    public class ConnectViewModel : INotifyPropertyChanged
    {
        ...
        private async void SetValues()
        {
            ...
            if(isOnDevice == null)
            {
                isOnDevice = new AppSettings();
                isOnDevice.Name = "IsOnDevice";
                isOnDevice.Value = "false";
                App.SQLiteDb.InsertAppSetting(isOnDevice);
            }
            if (isSettingsEmpty())
            {
        ...
        {
            var languageOnPhone = CultureInfo.CurrentUICulture.TwoLetterISOLanguageName;
            if(languageOnPhone != "pl")
            {
                await UserDialogs.Instance.ConfirmAsync("Aplikacja obsługuje tylko polską wersję językową.\nKorzystanie z innych może powodować błędy. \nTwoja aktualna to: "+languageOnPhone, "Nieobsługiwana wersja językowa!", "OK", "Anuluj");
                return;
            }
            ...
            {
               await Application.Current.MainPage.Navigation.PushModalAsync(new NavigationPage(new MainMenuPage()));
